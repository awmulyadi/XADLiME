...
from models import XADLiMEClassifer
from losses import *
from helpers import *
...
parser.add_argument("--classifier_dim", type=int, default=64)
...
parser.add_argument("--task", nargs='+', type=str, default=['CN', 'AD'])
# parser.add_argument("--task", nargs='+', type=str, default=['CN', 'MCI'])
# parser.add_argument("--task", nargs='+', type=str, default=['MCI', 'AD'])
# parser.add_argument("--task", nargs='+', type=str, default=['SMCI', 'PMCI'])
...
parser.add_argument("--note", type=str, default='XADLiME, Clinical Stage Classification')
...
if (args.som_map_size[0]==1 and args.som_map_size[1]==5 and args.som_map_size[2]==20):
    args.classifier_input_dim = 64 * 16
    args.scenario = '2D'
...
directory = 'log/XADLiME/%s/%s/XADLiME_%s_GPU%s_%d/' % (args.scenario, ('_').join(args.task), date_str, args.gpu_id, args.fold)
...
writelog(f, 'Classifier Input Dim: %d' % args.classifier_input_dim)
writelog(f, 'Classifier Dim: %d' % args.classifier_dim)
writelog(f, '----------------------')
writelog(f, 'WEIGHT')
writelog(f, 'weight_cons: %.5f' % args.weight_cons)
writelog(f, 'weight_ce: %.5f' % args.weight_ce)
writelog(f, '----------------------')
...
def train(dataloader, dir='.'):
    ...
    for i, xbatch in enumerate(tqdm(dataloader)):
        ...
        # Overall Loss
        loss_overall = loss_recon + loss_cons + loss_ce + loss_l1
        ...
        if len(args.task) == 2:
            y_prob = y_hat[:, 1].flatten().to('cpu').detach().numpy()
        else:
            y_prob = y_hat.to('cpu').detach().numpy()
        y_hat = torch.argmax(y_hat, axis=1).flatten().to('cpu').detach().numpy()
        if i == 0:
            y_hats = y_hat
            y_probs = y_prob
        else:
            ...
            if len(args.task) == 2:
                ...
                y_probs = np.vstack([y_probs, y_prob])
    ...
    writelog(f, 'Loss CE: %.8f' % loss_ce_)
    ...
    metric = calculate_performance(y_gts, y_hats, y_probs, args)
    metric_str = ['auc',
                  'auc_prc',
                  'acc',
                  'balacc',
...
def evaluate(phase, dataloader, dir='.'):
    ...
    with torch.no_grad():
        ...
        for i, xbatch in enumerate(tqdm(dataloader)):
            ...
            # Data
            x = normalize_gauss(device, xbatch['data'])
            idx = xbatch['image_id']
            y = xbatch['label'].long().to(device)
            d = xbatch['demographic'].float().to(device)
            batch_size = x.shape[0]
    ...
    return loss_overall_, \
           metric[('/').join(args.task)][0], \
           metric[('/').join(args.task)][2]
...
classifier = XADLiMEClassifer(args, n_output=len(args.task)).to(device)
criterion_recon = L1L2Loss(args).to(device)
criterion_cons = L1L2Loss(args).to(device)
criterion_ce = nn.CrossEntropyLoss().to(device)
params = list(synth.parameters()) + list(classifier.parameters())
optimizer = optim.Adam(params, lr=args.lr, weight_decay=args.lambda2)

# Best epoch checking
valid = {'epoch': 0, 'loss': 99999, 'auc': 0, 'acc': 0}
...
for epoch in range(args.epoch):
    ...
    loss, auc, acc = evaluate('valid', dataloaders['valid'], dir=directory)

    # Save Model
    if acc >= valid['acc']:
        torch.save(synth.state_dict(), directory + '/model/XADLiMESynth.pt')
        torch.save(classifier.state_dict(), directory + '/model/XADLiMEClassifer.pt')
        writelog(f, 'Best validation ACC is found! Validation ACC : %f' % acc)
    ...
    writelog(f, 'Test')
    _, _, _ = evaluate('test', dataloaders['test'], dir=directory)
...
classifier = XADLiMEClassifer(args, n_output=len(args.task)).to(device)
classifier.load_state_dict(torch.load(directory + '/model/XADLiMEClassifer.pt'))
